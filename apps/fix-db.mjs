...
async function main() {
  const cols = await sql`SELECT column_name FROM information_schema.columns WHERE table_name = 'seace_drafts' ORDER BY ordinal_position`;
  console.log('Columnas actuales:', cols.map(x => x.column_name));
